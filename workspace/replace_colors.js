const fs = require('fs');
const files = ['/workspace/src/App.tsx', '/workspace/src/components/Heatmap.tsx', '/workspace/src/components/CAVETest.tsx'];
...
function replaceColors(content) {
    ...
    content = content.replace(/STATUS_STYLE\['등장'\]/g, 'STATUS_STYLE[\'등장\']');

    // Fix active tab logic in Heatmap that we might break or broken
    content = content.replace(/bg-ui-bg-card text-ui-text-primary border-ui-border border-b-2/g, 'bg-ui-bg-card text-ui-text-primary border-ui-text-primary border-b-2');
    content = content.replace(/bg-ui-bg-card text-ui-text-secondary border-ui-border hover:bg-ui-bg-card/g, 'bg-ui-bg-main text-ui-text-secondary border-ui-border hover:bg-ui-bg-card');
